...
//    setupEvents / setValues 등의 함수가 공유하기 편하게 만들기 위해 멤버변수로 저장
    lateinit var mPizzaStore : Order
...
//        멤버변수로 있는 피자가게 변수를 intent 변수에서 받아온 가게로 대입.
        mPizzaStore = intent.getSerializableExtra("pizzaStore") as Order

//        피자가게변수를 통해 화면 데이터 세팅.
        storeName.text = mPizzaStore.name
        storeTel.text = mPizzaStore.phoneNum

//        이미지뷰에 => 웹에있는 그림 띄우기 Glide 활용
        Glide.with(mContext).load(mPizzaStore.logoUrl).into(storeLogo)
